...
// Images sourced from the open-source free-exercise-db (yuhonas/free-exercise-db)
// and ExerciseDB public CDN. Two images per exercise show start/end position.
...
const ExerciseDemoOverlay: React.FC<ExerciseDemoOverlayProps> = ({ exerciseKey, onStart, loading }) => {
  ...
  // Auto-cycle images every 1.8s to simulate animation
  useEffect(() => {
    setImgIndex(0);
    setImgError(false);
    if (demo.images.length < 2) return;
    const timer = setInterval(() => {
      setImgIndex(prev => (prev + 1) % demo.images.length);
    }, 1800);
    return () => clearInterval(timer);
  }, [exerciseKey, demo.images.length]);
    ...
    >
      ...
      <Box sx={{ position: 'relative', flex: '0 0 55%', overflow: 'hidden', bgcolor: '#050510' }}>
        {!imgError ? (
          ...
          // Fallback illustration when image fails to load
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 2 }}>
            <FitnessCenterIcon sx={{ fontSize: 80, color: '#00E5FF', opacity: 0.6 }} />
            <Typography variant="body2" color="rgba(255,255,255,0.5)" textAlign="center" sx={{ px: 2 }}>
              Exercise demonstration image unavailable
            </Typography>
          </Box>
        )}
            ...
            {demo.images.map((_, i) => (
              <Box
                ...
                sx={{
                  ...
                  height: 8,
                  borderRadius: 4,
                  bgcolor: i === imgIndex ? '#00E5FF' : 'rgba(255,255,255,0.3)',
                  transition: 'all 0.3s ease',
                  cursor: 'pointer',
                  boxShadow: i === imgIndex ? '0 0 8px #00E5FF' : 'none',
                }}
        ...
        {demo.images.length > 1 && (
          <>
            <IconButton
              onClick={handlePrev}
              sx={{ position: 'absolute', left: 4, top: '50%', transform: 'translateY(-50%)', color: 'white', bgcolor: 'rgba(0,0,0,0.45)', '&:hover': { bgcolor: 'rgba(0,229,255,0.25)' } }}
              size="small"
            >
            ...
            <IconButton
              onClick={handleNext}
              sx={{ position: 'absolute', right: 4, top: '50%', transform: 'translateY(-50%)', color: 'white', bgcolor: 'rgba(0,0,0,0.45)', '&:hover': { bgcolor: 'rgba(0,229,255,0.25)' } }}
              size="small"
            >
        ...
        <Chip
          ...
          sx={{
            ...
            bgcolor: 'rgba(0,0,0,0.7)', color: '#00E5FF',
        ...
        <Chip
          ...
          sx={{
            ...
            bgcolor: 'rgba(0,0,0,0.7)', color: '#FFD740',
      ...
      <Box sx={{ flex: 1, px: 2, py: 1.5, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.75, bgcolor: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.25)', borderRadius: 1.5, px: 1.25, py: 0.75 }}>
          ...
          <Typography variant="caption" color="#80DEEA" sx={{ lineHeight: 1.4 }}>
            <strong>Camera:</strong> {demo.cameraHint}
          </Typography>
        </Box>
      </Box>
    </Box>
